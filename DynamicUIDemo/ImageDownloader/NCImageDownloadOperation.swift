//
//  NCImageDownloadOperation.swift
//  DynamicUIDemo
//
...
class NCImageDownloadOperation: Operation {
    ...
    static var pathForDictionary: URL = {
        ...
        return urls[urls.count-1].appendingPathComponent("NCImageStore")
    }()
    ...
    func performOperation() {
        ...
        configuration.urlCache = NCImageDownloadOperation.sharedUrlCache
...
extension NCImageDownloadOperation: URLSessionDelegate, URLSessionDownloadDelegate, URLSessionTaskDelegate {
    @available(iOS 7.0, *)
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        var data: Data?
        if var compressData = self.getDataFromDisk(location: location) {
            if self.shouldStore {
                compressData = compressData.compressedImageData()
                if !FileManager.default.fileExists(atPath: NCImageDownloadOperation.pathForDictionary.path) {
                    do {
                        try FileManager.default.createDirectory(atPath: NCImageDownloadOperation.pathForDictionary.path, withIntermediateDirectories: true, attributes: nil)
                    }catch let error {
                        print(error)
                    }
                }
                let filePath = NCImageDownloadOperation.pathForDictionary.appendingPathComponent(self.urlString.md5)
