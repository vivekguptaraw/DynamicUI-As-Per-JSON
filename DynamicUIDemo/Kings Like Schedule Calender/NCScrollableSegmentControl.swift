//
//  NCScrollableSegmentControl.swift
//  DynamicUIDemo
//
//  Created by Vivek Gupta on 06/06/18.
...
@objc
public enum NCScrollableSegmentedControlSegmentStyle: Int {
    case textOnly, imageOnly, imageOnTop, imageOnLeft
}

/**
 A NCScrollableSegmentedControl object is horizontaly scrollable control made of multiple segments, each segment functioning as discrete button.
 */
@IBDesignable
@objc public class NCScrollableSegmentedControl: UIControl {
    ...
    @objc public var segmentStyle:NCScrollableSegmentedControlSegmentStyle = .textOnly {
        didSet {
    ...
    @objc public var selectedSegmentIndex: Int = -1 {
        willSet{
            ...
            if selectedSegmentIndex >= 0 {
                ...
                if let cell = collectionView?.cellForItem(at: indexPath) as? TextOnlySegmentCollectionViewCell{
                    print("will..\(cell.isSelected)")
                }
            }
        }
        didSet{
            ...
            if selectedSegmentIndex >= 0 {
                ...
                UIView.animate(withDuration: 0.5, animations: {
                   self.collectionView?.selectItem(at: indexPath, animated: false, scrollPosition: .left)
                    ...
                    if let cell = self.collectionView?.cellForItem(at: indexPath) as? TextOnlySegmentCollectionViewCell{
                        print("did..\(cell.isSelected)")
    ...
    func reloadSegments() {
        if let collectionView_ = collectionView {
            ...
            if selectedSegmentIndex >= 0 {
                let indexPath = IndexPath(item: selectedSegmentIndex, section: 0)
                if let cell = collectionView_.cellForItem(at: indexPath) as? TextOnlySegmentCollectionViewCell{
                    cell.titleLabel.textColor = selectedSegmentContentColor
                }
    ...
    final private class CollectionViewController : NSObject, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
        ...
        private weak var segmentedControl: NCScrollableSegmentedControl!
        
        init(segmentedControl:NCScrollableSegmentedControl) {
