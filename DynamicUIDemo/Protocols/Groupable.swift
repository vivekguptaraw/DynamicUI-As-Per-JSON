...
public extension Groupable {
    ...
    public func groupInCenter(group: Group, views: [Frameable], padding: CGFloat, width: CGFloat, height: CGFloat) {
        if views.count == 0 {
            print("Warning: No subviews provided to groupInCenter().")
            return
        }
    ...
    public func groupAgainstEdge(group: Group, views: [Frameable], againstEdge edge: Edge, padding: CGFloat, width: CGFloat, height: CGFloat) {
        if views.count == 0 {
            print("Warning: No subviews provided to groupAgainstEdge().")
            return
        }
    ...
    ///   - height: The height of each subview.
    ///
    public func groupAgainstEdge(group: Group, views: [Frameable], againstEdge edge: Edge, padding: CGFloat,width: CGFloat, widths: [CGFloat], height: CGFloat) {
        if views.count == 0 {
            print("Warning: No subviews provided to groupAgainstEdge().")
            return
        }
        ...
        case .right:
            if group == .horizontal {
                xOrigin = self.width - (CGFloat(views.count) * width) - (CGFloat(views.count) * padding) - (padding * CGFloat(views.count))
                yOrigin = (self.height / 2.0) - (height / 2.0)
                xAdjust = width + padding
            } else {
                xOrigin = self.width - width - padding
        ...
        var i: Int = 0
        for view in views {
            view.frame = CGRect(x:  xOrigin  , y: yOrigin, width: widths[i], height: height)
            
            xOrigin += view.frame.width + padding
            yOrigin += yAdjust
            i += 1
        }
    }
    
    //===========With unequal height of child
    /// Tell a view to group an array of its subviews against one of its edges, specifying the padding between each subview
    /// and their superview, as well as the size of each.
    ///
    /// - parameters:
    ...
    ///   - height: The height of each subview.
    ///
    public func groupAgainstEdge(group: Group, views: [Frameable], againstEdge edge: Edge, padding: CGFloat,width: CGFloat, heights: [CGFloat], height: CGFloat) {
        if views.count == 0 {
            print("Warning: No subviews provided to groupAgainstEdge().")
            return
        }
        ...
        var xAdjust : CGFloat = 0.0
        var yAdjust : CGFloat = 0.0
        var totalHeight = heights.reduce(0, +)
        ...
        case .left:
            if group == .horizontal {
                ...
                yOrigin = (self.height - totalHeight - (CGFloat(views.count - 1) * padding)) / 2.0
                yAdjust = height + padding
            }
        ...
        var i: Int = 0
        for view in views {
            view.frame = CGRect(x:  xOrigin  , y: yOrigin, width: width, height: heights[i])
            
            xOrigin += xAdjust
            yOrigin += view.frame.height + padding //yAdjust
            i += 1
        }
    }
    
    //===========With unequal height and width of child
    /// Tell a view to group an array of its subviews against one of its edges, specifying the padding between each subview
    /// and their superview, as well as the size of each.
    ///
    /// - parameters:
    ...
    ///   - height: The height of each subview.
    ///
    public func groupAgainstEdgeWithUnequalHeightWidth(group: Group, views: [Frameable], againstEdge edge: Edge, padding: CGFloat,width: CGFloat, heights: [CGFloat], widths: [CGFloat], height: CGFloat) {
        if views.count == 0 {
            print("Warning: No subviews provided to groupAgainstEdge().")
            return
        }
        ...
        case .left:
            ...
            if group == .horizontal {
                yOrigin = (self.height / 2.0) - (heights[0] / 2.0)
                xAdjust = width + padding
            } else {
                yOrigin = (self.height - totalHeight - (CGFloat(views.count - 1) * padding)) / 2.0
                yAdjust = height + padding
...
//            } else {
//                yOrigin = padding
//                xOrigin = (self.width / 2.0) - (widths[0] / 2.0)
//                //yAdjust = height + padding
//            }
        ...
        if group == .horizontal {
            for view in views {
                yOrigin = (self.height / 2.0) - (heights[i] / 2.0)
                
                view.frame = CGRect(x: xOrigin, y: yOrigin, width: widths[i], height: heights[i])
                
                
                xAdjust = widths[i] + padding
            ...
            for view in views {
                view.frame = CGRect(x:  (self.width / 2.0) - (widths[i] / 2.0)  , y: yOrigin, width: widths[i], height: heights[i])
                
                xOrigin += xAdjust
                yOrigin += view.frame.height + padding
                i += 1
            }
        }
    ...
    public func groupAndFill(group: Group, views: [Frameable], padding: CGFloat) {
        if views.count == 0 {
            print("Warning: No subviews provided to groupAndFill().")
            return
        }
    ...
    fileprivate func groupInCornerHorizontal(views: [Frameable], inCorner corner: Corner, padding: CGFloat, width: CGFloat, height: CGFloat) {
        if views.count == 0 {
            print("Warning: No subviews provided to groupInCorner().")
            return
        }
    ...
    fileprivate func groupAndAlignHorizontal(align: Align, views: [Frameable], relativeTo sibling: Frameable, padding: CGFloat, width: CGFloat, height: CGFloat) {
        if views.count == 0 {
            print("Warning: No subviews provided to groupAndAlign().")
            return
        }
